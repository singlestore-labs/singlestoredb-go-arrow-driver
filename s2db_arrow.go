...
func (s2db *S2DBArrow) Execute(ctx context.Context, recordSize int64, query string, args ...interface{}) error {
	...
	for index, col := range cols {
		s2db.columnHandlers[index], err = column_handler.GetColumnHandler(index, col)
		if err != nil {
			return err
		}
...
func (s2db *S2DBArrow) GetNextArrowRecordBatch() (array.Record, error) {
	...
	for ; rowsRead < s2db.recordSize && s2db.rows.Next(); rowsRead++ {
		variables := make([]interface{}, len(s2db.columnHandlers))
		for i, handler := range s2db.columnHandlers {
			variables[i] = handler.GetVariable()
		}

		err = s2db.rows.Scan(variables...)
		if err != nil {
		...
		for _, handler := range s2db.columnHandlers {
			handler.AppendValue(s2db.recordBuilder)
		}
